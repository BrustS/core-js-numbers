...
function roundToPowerOfTen(num, pow) {
  if (pow === 0) return num;
  const elem = 10 ** pow;
  const rest = num - (num % elem);
  return num % elem > 4 * 10 ** (pow - 1) ? rest + elem : rest;
}
...
function isPowerOfTwo(num) {
  return num - 2 ** Math.round(Math.log2(num)) === 0;
}
